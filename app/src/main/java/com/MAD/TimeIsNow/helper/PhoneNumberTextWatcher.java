...
public class PhoneNumberTextWatcher implements TextWatcher{
    ...
    public PhoneNumberTextWatcher(TextInputLayout edTxtPhone) {
        this.editText = edTxtPhone.getEditText();
    }
        ...
        if (before == 0 && count == 1) {  //Entering values
            ...
            if (val != null && val.length() > 0) {
                ...
                if (c != null && c.length() > 0) {
                    stringBuffer.append("-");
                    if(c.length() <= 4) stringBuffer.append(c);
                    else stringBuffer.append(c.substring(0,4));
                }
